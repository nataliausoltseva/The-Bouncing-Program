...
public abstract class MovingShape {
    ...
    protected static final int defaultWidth = 50, defaultHeight = 50, defaultMarginWidth = 800,
            defaultMarginHeight = 500;
     ...
     */
    public MovingShape() {
        this(0, 0, defaultWidth, defaultHeight, defaultMarginWidth, defaultMarginHeight, defaultBorderColor,
                defaultFillColor, defaultPath); // the default properties
    }
     ...
     * 
     * @param pathID the integer value of the path MovingPath.FALLING is the falling
     *               path
     */
    ...
    /*
     * =============================================================================
     * == MovingPath : The superclass of all paths. It is an inner class. A path can
     * change the current position of the shape.
     * =============================================================================
     * ==
     */
    ...
    public class MyMovingPath extends MovingPath {
        private boolean movingRight;
        private int remainingMovingDownSteps;
        private int delta;
         ...
         */
        public MyMovingPath() {
            movingRight = new Random().nextInt(2) == 0;
            remainingMovingDownSteps = 0;
            delta = 5;
        }
        ...
        public void move() {
            int newTopLeftX;
            if (movingRight) {
                newTopLeftX = topLeft.x + delta;
            } else {
                newTopLeftX = topLeft.x - delta;
            }

            if (newTopLeftX + width > marginWidth) {
                topLeft.x = marginWidth - width;
                remainingMovingDownSteps = height / delta;
                movingRight = false;
                return;
            }
            ...
            if (remainingMovingDownSteps > 0) {
                if (topLeft.y > marginHeight) { // if it reaches the bottom of the frame, start again from the top
                    topLeft.y = 0;
                    remainingMovingDownSteps--;
                    return;
                }
                topLeft.y = topLeft.y + delta;
                remainingMovingDownSteps--;
            } else {
                topLeft.x = newTopLeftX;
            }
